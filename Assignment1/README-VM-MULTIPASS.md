...
```bash
cd comnet-assignments-fall25/Assignment1
...
Then add a new host entry like this: `ubuntu@<VM IP address>` and try connecting to the VM. After the connection is established (usually takes 30 seconds), open the file explorer and select the comnet-assignments-fall25 directory. 
...
To use this method, create a private repository for yourself, copy the reposory address and use the following command in the `comnet-assignments-fall25` directory inside the VM to add a new remote:
